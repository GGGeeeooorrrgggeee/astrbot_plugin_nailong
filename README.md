...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Version-3.0.1-orange" alt="Version">
...
插件指令会跟随 AstrBot 设置的唤醒词。例如 AstrBot 唤醒词是 `/`，则使用 `/来只奶龙`；如果唤醒词是 `#`，则使用 `#来只奶龙`。
...
5. 自动发送会过滤指令消息；如果其他插件使用完全无唤醒词的自然语言指令，且 AstrBot 没有把它标记为已识别指令，仍可能被当作普通聊天参与概率判断。
