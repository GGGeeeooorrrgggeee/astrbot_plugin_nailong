...
| 查询奶龙文件名        | 支持通过发送或回复图库里的表情包查看本地文件名，支持批量查询 |
...
| `删除奶龙`(可批量)       | 文件名(可选) 或回复表情包     | 带文件名时删除单张；不带文件名时按表情包哈希值匹配本地图库，回复或消息中带多张表情包时会批量删除 |
| `查询奶龙文件名`(可批量) | 回复表情包或消息中带表情包    | 查看图库中对应表情包的本地文件名；一次带多张表情包时会批量返回文件名 |
| `重命名奶龙`             | 新文件名 或 原文件名 新文件名 | 带表情包或回复表情包时使用 `重命名奶龙 新文件名`；不带表情包时使用 `重命名奶龙 原文件名 新文件名`；一次只重命名一张 |
...
4. `覆盖下载` 会先清空当前本地图库和哈希记录，再导入本次下载到的表情包；`追加下载` 会保留已有文件并追加新文件。
